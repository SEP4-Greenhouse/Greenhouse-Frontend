// __tests__/historyService.test.ts
import { fetchHistoricalSensorData } from "../historyService";
...
describe("fetchHistoricalSensorData", () => {
  ...
  it("returns mapped historical sensor data", async () => {
    const mockData = [
      ...
      { sensorId: 999, timeStamp: "2024-01-01T02:00:00Z", value: 99 }, // filtered out
    ];
  ...
  it("throws if request fails", async () => {
    ...
    await expect(
      fetchHistoricalSensorData(sensorType, "day")
    ).rejects.toThrow(`Failed to fetch ${sensorType} history for day`);
  });
});
